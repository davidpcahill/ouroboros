...
def setup_logging():
    ...
    handler = RotatingFileHandler('ouroboros.log', maxBytes=10000000, backupCount=5)
...
def get_ai_response(prompt):
    try:
        client = anthropic.Client(api_key=config.get('Anthropic', 'API_KEY'))
    ...
    try:
        ...
        for action_count in range(1, max_actions + 1):
            ...
            if ai_response.startswith('[DOCKERFILE]'):
                ...
                with open(code_path, 'w') as f:
                    f.write(code)
                results = run_in_docker(docker_client, current_dockerfile, code)
...
def main():
    ...
    schedule.every(int(config.get('Scheduling', 'IntervalMinutes', fallback='60'))).minutes.do(run_experiment_cycle, docker_client)
